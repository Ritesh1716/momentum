const CACHE_NAME = 'momentum-v113';

// CDN URLs to cache on first load (Fix 9 — cache CDN files)
const CDN_CACHE = 'momentum-cdn-v86';
const CDN_URLS = [
...
const FONT_CACHE = 'momentum-fonts-v85';
  ...
  event.waitUntil(
    Promise.allSettled([
      ...
      caches.open(CACHE_NAME).then(cache =>
        Promise.allSettled(['/', '/manifest.json', '/icon-192.png', '/icon-512.png', '/mo-meditate.png'].map(url => cache.add(url)))
      ),
...
self.addEventListener('fetch', event => {
  ...
  // Skip non-http(s) requests (e.g. chrome-extension://) — Cache API doesn't support these schemes
