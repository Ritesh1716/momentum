const CACHE_NAME = 'momentum-v171';
...
self.addEventListener('fetch', event => {
  ...
  // Mo character images — network-first with a hard cache bypass.
  // IMPORTANT: Only apply cache:'reload' when the URL has our ?v= version
  // stamp (added by MoImg component). The splash screen uses bare /mo/*.png
  // URLs (no version param) and needs to load from cache for instant draw.
  if (url.pathname.startsWith('/mo/') && url.pathname.endsWith('.png')) {
    const hasVersionParam = url.search.includes('v=');
    event.respondWith(
      fetch(event.request.url, hasVersionParam ? { cache: 'reload' } : {}).then(resp => {
        if (resp.ok) {
          const clone = resp.clone();
          caches.open(CACHE_NAME).then(cache => cache.put(event.request, clone));
        }
